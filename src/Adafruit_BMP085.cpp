...
void Adafruit_BMP085::write8(uint8_t a, uint8_t d) {
  Wire.beginTransmission(BMP085_I2CADDR); // start transmission to device
#if (ARDUINO >= 100)
//  Wire.writeMe(a); // sends register address to read from
//  Wire.writeMe(d);  // write data
  Wire.write((int)a); // sends register address to read from
  Wire.write((int)d);  // write data
#else
  Wire.send(a); // sends register address to read from
